...
async def create_new_chat(message: Message):
    ...
    await message.answer(
        f"Начат новый чат (ID: {chat_id}). Пишите!", keyboard=MAIN_MENU_KEYBOARD
    )
...
async def chat_message_handler(message: Message):
    ...
    if not active_chat_id:
        ...
        await message.answer(
            "Создан новый чат. Обрабатываю запрос...", keyboard=MAIN_MENU_KEYBOARD
        )
    ...
    response = await ai_service.get_ai_response(
        user_id, active_chat_id, user_text, photo_url=photo_url
    )
